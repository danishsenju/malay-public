...
**Exception:** `.plate` / `.pressable` use a hard 4px offset shadow (`4px 4px 0 0` ink-black, no blur) as a deliberate neo-brutalist press effect, established in Phase 1-4. Pressing collapses the shadow while the element travels the same 4px so it lands flush - a physical "push into the page." This is intentional, not a lapse of the border-is-elevation rule. It stays a _hard, non-blurred_ offset - soft/blurred drop shadows remain forbidden.

**Performance note:** the built `.pressable` transitions `box-shadow` (a paint property, off-GPU). New components should instead animate the offset shadow via an opacity-transitioned pseudo-element layer, keeping motion on transform/opacity only. The already-shipped `.plate`/`.pressable` are not retrofitted - verified and working.

**Known gap (revisit post-submission):** the global `prefers-reduced-motion` block zeroes _all_ transition-duration, which also flattens opacity/color fades. Emil's guidance is gentler-not-zero (keep opacity/color, drop movement). Left as-is this close to the deadline to avoid touching the verified global animation block.
...
:root {
  ...
  --font-linksans:
    "Linksans", ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont,
    "Segoe UI", Roboto, sans-serif;
  --font-linksans-linksansvf:
    "Linksans Linksansvf", ui-sans-serif, system-ui, -apple-system,
    BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif;
  --font-yerk:
    "yerk", ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont,
    "Segoe UI", Roboto, sans-serif;
